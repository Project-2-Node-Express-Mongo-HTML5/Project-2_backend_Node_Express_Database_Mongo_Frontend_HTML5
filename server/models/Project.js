...
class Project {
  ...
  static validate(data) {
    ...
    if (!data.intrinsicPriority || data.intrinsicPriority < 1 || data.intrinsicPriority > 10) {
      errors.push('Priority must be between 1 and 10');
    }

    if (data.status && !['active', 'completed', 'abandoned', 'archived'].includes(data.status)) {
      errors.push('Invalid status');
    }

    if (data.season && Array.isArray(data.season)) {
      const validSeasons = ['spring', 'summer', 'fall', 'winter'];
      const invalidSeasons = data.season.filter(s => !validSeasons.includes(s));
  ...
  // Transform for output (adds virtual fields)
  ...
  static sanitize(data) {
    return {
      ...
      tags: Array.isArray(data.tags) ? data.tags.map(t => t.trim()).filter(t => t) : [],
