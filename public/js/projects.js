...
async function loadProjects() {
  try {
    ...
    if (sortByPriority) {
      ...
      url = `${API_URL}?status=${currentFilter}`;
    }
    ...
    <div class="project-card">
      <div class="project-header">
        ...
        <span class="project-priority">
          Priority: ${project.intrinsicPriority}
        </span>
      </div>
      ...
      <div class="project-meta">
        ...
        <div class="meta-item">
          <span class="effort-badge effort-${project.effortLevel}">
            ${capitalize(project.effortLevel)} Effort
          </span>
        </div>
      ...
      ${
        project.season?.length
          ? `
        <div class="project-tags">
          ${project.season
            .map(
              (s) => `
            <span class="tag"> ${capitalize(s)}</span>
          `,
            )
            .join("")}
        </div>
      ...
      <div class="project-actions">
        ${
          ...
          <button class="btn btn-danger btn-small"
            onclick="abandonProject('${project._id}')">
            ✗ Abandon
          </button>
        `
            : ""
...
async function handleFormSubmit(e) {
  ...
  const formData = {
    ...
    tags: document
      ...
      .filter(Boolean),
    season: Array.from(
      document.querySelectorAll('input[name="season"]:checked'),
    ).map((cb) => cb.value),
  };
...
async function abandonProject(id) {
  ...
  await fetch(`${API_URL}/${id}/abandon`, { method: "PATCH" });
  loadProjects();
}
